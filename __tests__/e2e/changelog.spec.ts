...
test.describe('Changelog tests', () => {
  ...
  test('Load product selector', async ({ page }) => {
    ...
    await expect(page.getByRole('option', { name: 'Content Hub', exact: true })).toBeVisible();
    ...
    await expect(page.getByRole('option', { name: 'Cloud Portal' })).toBeVisible();
  ...
  test('Load changetype selector', async ({ page }) => {
    ...
    await expect(page.getByRole('option', { name: 'New feature' })).toBeVisible();
