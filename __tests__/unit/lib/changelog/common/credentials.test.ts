...
describe('getChangelogCredentials', () => {
  ...
  test('should construct credentials from environment variables', () => {
    ...
    expect(credentials.production.endpoint).toBe('https://api.example.com/tenant123/prod');
    expect(credentials.production.token).toBe('delivery-token');
    expect(credentials.preview.endpoint).toBe('https://api.example.com/tenant123/prod?preview=true');
  ...
  test('should handle different endpoint formats', () => {
    ...
    expect(credentials.production.endpoint).toBe('https://api.sitecore.com/my-tenant/dev');
    expect(credentials.preview.endpoint).toBe('https://api.sitecore.com/my-tenant/dev?preview=true');
  });
  ...
  test('should construct production endpoint correctly', () => {
    ...
    expect(credentials.production.endpoint).toContain('https://api.example.com');
