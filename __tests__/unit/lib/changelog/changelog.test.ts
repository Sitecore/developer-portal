...
import { ChangelogNotFoundError, ChangelogValidationError } from '@lib/changelog/errors';
import { ChangelogCredentials } from '@lib/changelog/types';
import { mockedChangelogEntries, mockedChangelogEntry } from '__mocks__/changelog/changelogEntry.mock';
...
import { getCachedEntryCount, requestDeduplicator, setCachedEntryCount } from '@lib/changelog/cache';
import { fetchGraphQL } from '@lib/changelog/common/fetch';
import { parseChangeLogItem, ParseRawData } from '@lib/changelog/types/changeLogEntry';
...
describe('Changelog', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(requestDeduplicator.execute).mockImplementation(async (key, fn) => fn());
  });
  ...
  describe('getEntryByTitleAndDate', () => {
    ...
    test('should get entry by title and date with legacy signature', async () => {
      ...
      const result = await changelog.getEntryByTitleAndDate('Test Entry', '12072024', 'product-123');
    ...
    test('should throw ChangelogValidationError with legacy signature when date or productId missing', async () => {
      ...
      await expect(changelog.getEntryByTitleAndDate('Test Entry', undefined, 'product-123')).rejects.toThrow(ChangelogValidationError);
      await expect(changelog.getEntryByTitleAndDate('Test Entry', '12072024', undefined)).rejects.toThrow(ChangelogValidationError);
    });
  ...
  describe('getEntryByTitle', () => {
    ...
    test('should get entry by title with legacy signature', async () => {
      ...
      const result = await changelog.getEntryByTitle('Test Entry', 'product-123');
  ...
  describe('getEntriesPaginated', () => {
    test('should call getEntries with correct parameters', async () => {
      ...
      const result = await changelog.getEntriesPaginated('10', 'product-123', 'change-123', 'cursor-123', true);
  ...
  describe('getEntries', () => {
    ...
    test('should get entries with pagination', async () => {
      ...
      const result = await changelog.getEntries({ pageSize: 10, endCursor: 'cursor-123' });
  ...
  describe('getStatus', () => {
    test('should get statuses', async () => {
      const changelog = new Changelog(mockCredentials);
      const mockStatuses = [
        { id: '1', name: 'Available', identifier: 'available', description: '' },
        { id: '2', name: 'Coming Soon', identifier: 'coming-soon', description: '' },
      ];
  ...
  describe('getProducts', () => {
    test('should get products with entry counts in production mode', async () => {
      ...
      const mockProducts = [
        { id: '1', name: 'Product A', darkIcon: '', lightIcon: '', hasEntries: false },
        { id: '2', name: 'Product B', darkIcon: '', lightIcon: '', hasEntries: false },
      ];
    ...
    test('should get products with hasEntries true in preview mode', async () => {
      const changelog = new Changelog(mockCredentials, true);
      const mockProducts = [{ id: '1', name: 'Product A', darkIcon: '', lightIcon: '', hasEntries: false }];
...
describe('GetEntryCountByProductId', () => {
  ...
  test('should return cached count if available', async () => {
    ...
    const result = await GetEntryCountByProductId(mockCredentials, 'product-123', false);
  ...
  test('should check cache again after deduplication', async () => {
    vi.mocked(getCachedEntryCount).mockReturnValueOnce(null).mockReturnValueOnce(10);
    vi.mocked(fetchGraphQL).mockResolvedValue({
      data: {
        changelog: {
          results: [{}],
