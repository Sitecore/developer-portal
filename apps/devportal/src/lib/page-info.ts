// Global
import type { ChildPageInfo, MarkdownMeta, PageInfo, PagePartialGroup, PagePartials, PartialData, SidebarNavigation } from '@lib/interfaces/page-info';
import fs from 'fs';
...
export const getChildNavgationInfo = async (currentUrlSegment: string): Promise<SidebarNavigation | undefined> => {
  const manifest = searchForFile(path.join(pagesDirectory, currentUrlSegment), 'manifest.json');
  if (manifest != null) {
    const fileData: SidebarNavigation = JSON.parse(fs.readFileSync(manifest, { encoding: 'utf-8' }));
    return fileData;
  }
};
