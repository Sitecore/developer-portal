...
export const ChangelogItemMeta = ({ item, loading }: ChangelogItemMetaProps) => {
  return (
    <div className="my-3 flex flex-row items-center space-x-3 text-gray-500 dark:text-gray-400">
      ...
      <div className="flex flex-row gap-5">
        <span className={`changetype bg-theme-bg-alt flex items-center justify-center text-xs ${loading ? 'w-12' && skeletonLoaderClasses : `rounded-md px-3 py-1 ${slugify(item.changeType[0].name)} `}`}>{item.changeType[0].name}</span>
