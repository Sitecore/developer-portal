...
const ChangeLogItem = ({ item, loading }: ChangeLogItemProps): JSX.Element => {
  return (
    <div className=" bg-theme-bg mt-8 mb-16">
      ...
      {item.image.length > 0 && (
        <div className={`relative my-4 h-40 w-full rounded-lg ${loading ? 'w-12' && skeletonLoaderClasses : ''}`}>
          <Image
      ...
      <div className="mt-3 flex items-center space-x-3 text-gray-500 dark:text-gray-400">
        <div className="flex items-center gap-5">
          ...
          <div>
            <span className="bg-theme-bg-alt inline-block rounded-md px-3 py-1 text-sm ">{item.changeType[0].name}</span>
          </div>
        </div>
      </div>
