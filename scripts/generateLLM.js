...
function getAllRoutes(manifest, parentPath = '', rootPath = '') {
  ...
  if (manifest.routes) {
    manifest.routes.forEach((route) => {
      ...
      if (!route.ignoreLink) {
        const fullPath = rootPath ? path.join(rootPath, currentPath).replace(/\\/g, '/') : currentPath;
      ...
      if (route.children) {
        const childRoutes = getAllRoutes({ routes: route.children }, currentPath, rootPath);
...
function generateRouteList(manifestPath) {
  ...
  const markdownLinks = routes.map((route) => `- [${route.title}](${BASE_URL}${route.path})`).join('\n');
