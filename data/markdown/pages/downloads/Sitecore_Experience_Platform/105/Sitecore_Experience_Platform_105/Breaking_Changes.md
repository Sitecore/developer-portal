...
| Context | Description | Ref |
| --- | --- | --- |
| API | The public property `ICollection<string> UntokenizedSolrTypes` has been removed. The set of non-tokenizable Solr field types is now owned by the new `Sitecore.ContentSearch.Linq.Solr.Parsing.ITokenizableSolrFieldTypeResolver` service, resolved from the Sitecore DI container, and `protected virtual bool ShouldTokenize(TextQueryTranslatorContext)` now delegates to `ITokenizableSolrFieldTypeResolver.IsTokenizable(string)`. The default implementation treats `string`, `lowercase`, and the new `lowercase_sort` as non-tokenizable. The `protected BaseTokenizableValueSolrTextQueryTranslator(BaseSolrTokenizer)` constructor keeps its signature but now requires `ITokenizableSolrFieldTypeResolver` to be registered in the container. Constructing a translator outside an initialized Sitecore DI context will now throw. To change which field types are tokenized, register your own `ITokenizableSolrFieldTypeResolver` instead of mutating `UntokenizedSolrTypes`. Affects: solutions that subclass any Solr text query translator, that read or mutate `UntokenizedSolrTypes`, or that instantiate translators in unit tests. Removal of the property produces compilation error `CS0117`. | PDXP-26340 |

#### Sitecore.ContentSearch.SolrProvider.Pipelines.FormatQueryFieldValue.ApplyFieldMappingRule

| Context | Description | Ref |
| --- | --- | --- |
| API | `ApplyFieldMappingRule` (already deprecated in 10.1) has gained a public parameterless constructor that resolves `ITokenizableSolrFieldTypeResolver` from the service provider, and it now requires that service to be registered. Its field value escaping logic no longer switches on a hardcoded list of schema type names. Instead, it asks the resolver whether the schema type is tokenizable. As a consequence, a field whose schema type is tokenizable but is neither `text` nor `text_general` (which now includes `text_gen_sort` and every `text_*` language type) falls through to the "no escaping" branch and returns early, where previously only types outside the hardcoded list did. Affects: solutions that still have this processor in the `contentSearch.formatQueryFieldValue` pipeline, or that instantiate it directly. | PDXP-26340 |

#### Sitecore.ContentSearch.SolrProvider.SolrQueryOptionsBuilder.PartBuilders.FacetPartBuilder

| Context | Description | Ref |
| --- | --- | --- |
| API | The public constructor `FacetPartBuilder(BaseCorePipelineManager)` has been replaced by `FacetPartBuilder(BaseCorePipelineManager, BaseSolrSpecificSettings)`. The builder uses the settings instance to decide whether to append `facet.method=enum` to `QueryOptions.ExtraParams`. Affects: solutions that construct `FacetPartBuilder` directly, or that subclass `DefaultQueryOptionsPartBuildersFactory` and override part builder creation. A call to the old constructor produces compilation error `CS1729`. | PDXP-26340 |

#### Sitecore.ContentSearch.SolrProvider.Abstractions.BaseSolrSpecificSettings

| Context | Description | Ref |
| --- | --- | --- |
| API | A new `internal virtual bool UseEnumFacetMethod()` member has been added to `BaseSolrSpecificSettings`, returning `false` by default, and overridden in `DefaultSolrSpecificSettings` to read the `ContentSearch.Solr.UseEnumFacetMethod` setting. The member is `internal`, so solutions that subclass `BaseSolrSpecificSettings` from outside the assembly cannot override it and will always get the default `false`. Affects: solutions that subclass `BaseSolrSpecificSettings` and need to control the facet method programmatically. | PDXP-26340 |

#### Sitecore.ContentSearch.AbstractDocumentBuilder

| Context | Description | Ref |
| --- | --- | --- |
| API | The deprecated `protected virtual ParallelDisabledSecurityProxy ParallelForeachSiteContextSwitcher` property has been renamed to `ParallelForeachProxy`. It remains deprecated, unused, and slated for removal. If your solution subclasses `AbstractDocumentBuilder` and overrides the old property, the override is silently no longer invoked, so remove it. Affects: solutions that subclass `AbstractDocumentBuilder`. Search your codebase for `ParallelForeachSiteContextSwitcher`. | PDXP-26340 |

#### Sitecore.ContentSearch.SolrNetExtension.Commands.SchemaCommand

| Context | Description | Ref |
| --- | --- | --- |
| API | `SchemaCommand` is now a `partial` class. Its JSON serialization has been corrected: `JsonConvert.SerializeXNode` does not emit a JSON array when the source XML has only one child element, which Solr 10's Schema API rejects. The new internal `SolrSchemaJsonHelper.EnsureSingleObjectInArrays` post-processes the serialized payload so that single-element collections are emitted as arrays. Solutions that captured or asserted on the exact JSON body sent to the Schema API will see a different payload. Affects: solutions with tests or proxies that inspect Schema API request bodies. | PDXP-26340 |

#### Sitecore.ContentSearch.SolrProvider.SolrNetIntegration.DefaultSolrLocator

| Context | Description | Ref |
| --- | --- | --- |
| API | `DefaultSolrLocator` now registers `BaseSolrAdmin` in the SolrNet container, backed by a `SolrAdmin` instance built over the admin connection. This is consumed by the new internal `ISolrVersionResolver` / `SolrVersionResolver` pair, which `ResolveSchemaName` uses to determine the Solr version. Solutions that replace `DefaultSolrLocator` must register `BaseSolrAdmin`, or `ResolveSchemaName` will fail to resolve and Schema Populate will throw. Affects: solutions that replace or subclass `DefaultSolrLocator`. | PDXP-26340 |

#### Dependency injection registrations

| Context | Description | Ref |
| --- | --- | --- |
| API / Dependencies | `ContentSearchServicesConfigurator` registers five new singletons: `ITokenizableSolrFieldTypeResolver` → `DefaultTokenizableSolrFieldTypeResolver`, `ISolrVersionResolver` → `SolrVersionResolver`, `DefaultNamedSchemaPopulateHelperFactory`, and both `INamedSchemaPopulateHelperFactory` and `ISolrVersionToSchemaNameTranslator` resolving to that same factory instance. `ITokenizableSolrFieldTypeResolver` and `INamedSchemaPopulateHelperFactory` are required by types with public parameterless constructors (`ApplyFieldMappingRule`, `BaseTokenizableValueSolrTextQueryTranslator`, `PopulateFields`, `ResolveSchemaName`), so those types now throw when constructed outside an initialized Sitecore DI container. Affects: solutions that replace `ContentSearchServicesConfigurator`, or that unit-test Solr provider types without a configured service provider. | PDXP-26340 |
