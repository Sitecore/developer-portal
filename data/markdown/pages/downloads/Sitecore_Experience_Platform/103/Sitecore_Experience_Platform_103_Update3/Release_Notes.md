...
| Sitecore Kernel | When multiple threads attempt to add entries to the HTML cache simultaneously using HtmlCache.AddResilient(...), a thread-safety issue in the internal use of List<T> causes an IndexOutOfRangeException. This results in application instability under load and may require a restart to recover. | 536792 |
