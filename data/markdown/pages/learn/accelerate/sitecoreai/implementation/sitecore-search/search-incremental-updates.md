...
description: 'Update Search indexes based on content updates in SitecoreAI'
hasSubPageNav: true
hasInPageNav: true
area: ['accelerate']
created: '2025-02-27'
...
This article discusses how to integrate Sitecore Search with SitecoreAI. If you need to integrate a different search or content provider, please consider these techniques when deciding if they can be applied to your scenario.
...
### SitecoreAI & Search setup

First we need to make sure that the meta data is added to the header in SitecoreAI, for example:
...
<li>Web pages generated by the front-end application often use multiple, external datasources in addition to the content inside of SitecoreAI, in which case you would need to retrieve this additional information from the middleware and update the documents in Search accordingly. </li>
...
A basic solution is to define a new page property in SitecoreAI, name it `Reindex` (boolean): 
...
Consider that these page attributes do not change the way SitecoreAI publishes, nor how Experience Edge notifies the middleware via webhook; they are just attributes like `name` and `description`. `Reindex` and `ReindexDescendants `could be used by the middleware to make a decision on how to perform incremental update against Search.
...
A more clever middleware could instead observe a *stateful approach* and leverage on the SitecoreAI [webhook notifications](https://doc.sitecore.com/xmc/en/developers/xm-cloud/webhook-event-handler-configuration-fields.html#supported-events) in addition to Experience Edge notifications. The middleware could use the `publish:begin` event (SitecoreAI notification) to understand if it is a full site publishing:
