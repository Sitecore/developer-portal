  ...
  return (
    <div>
      <div className="mb-8 justify-between md:flex md:items-end">
        ...
        <Button variant="text" icon={true} href={`https://twitter.com/${handle.substring(1, handle.length)}`} text="See more on Twitter" target="_blank" className="ml-auto" />
      </div>
      <ul className="grid gap-6 md:grid-cols-3">
        {content.map((tweet) => (
          <li key={tweet.id} className="-mb-2 -mt-2 block">
            <TwitterTweetEmbed tweetId={tweet.id} options={{ cards: 'hidden' }} placeholder={<div className="aspect-w-3 aspect-h-4 bg-gray-lightest w-full rounded-lg"></div>} />
          </li>
        ))}
      </ul>
    </div>
