  ...
  return (
    <ExtendedCard className={cn('p-0', className)} style="flat" elevation="none">
      <CardHeader className="flex justify-between py-8 px-0">
        <h3 className="text-2xl font-heading font-medium">{title ? title : `Latest ${playlistTitle} videos`}</h3>
...
export const YouTubeItem = ({ snippet, id }: YouTubeItemProps) => {
  ...
  return (
    <div key={id}>
      <NextLink href={`https://www.youtube.com/watch?v=${snippet.resourceId.videoId}&list=${snippet.playlistId}`} target="_blank" rel="noopener noreferrer" className="block">
        <div className="relative mb-4">
          <span className="sr-only">Opens in a new tab</span>
          <div style={{ aspectRatio: aspectRatio.toString() }} className="relative">
            <Image src={snippet.thumbnails.medium.url} alt={snippet.title} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="rounded-md object-cover" />
          </div>
          <div className="absolute inset-0 flex items-center justify-center">
            <svg width="58" height="59" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" aria-label="Play video" role="img">
