...
export class SitecoreCommunityApi {
  ...
  static async getEvents(maxResults: number): Promise<Array<SitecoreCommunityEvent>> {
    ...
    return fetch(`https://community.sitecore.com/api/sn_communities/v1/community/events?${params.join('&')}`).then(async (response) => {
      if (!response.ok) {
  ...
  static async getContent({ contentType, maxResults, sort, forum }: SitecoreCommunityOptions): Promise<Array<SitecoreCommunityContent>> {
    ...
    if (contentType) {
      params.push(`type=${SitecoreCommunityApi.CONTENT_TYPE_IDS[contentType]}`);
    }
    ...
    return fetch(`https://community.sitecore.com/api/sn_communities/v1/community/contents?${params.join('&')}`)
      ...
      .catch((err) => {
        // On error, log and return an empty array so callers receive a serializable value
        // and the page build does not fail due to non-serializable Error objects.
        // eslint-disable-next-line no-console
        console.error('SitecoreCommunityApi.getContent error:', err);
        return [] as Array<SitecoreCommunityContent>;
      });
  }
  ...
  static async get({ contentType = undefined, forum = undefined, maxResults = 3, sort = 'created' as SortOption }: SitecoreCommunityOptions): Promise<Array<SitecoreCommunityEvent> | Array<SitecoreCommunityContent>> {
    ...
    return SitecoreCommunityApi.getContent({
      contentType,
      maxResults: count,
      sort,
      forum,
