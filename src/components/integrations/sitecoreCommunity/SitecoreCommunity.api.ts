...
export class SitecoreCommunityApi {
	...
	static async getEvents(
		maxResults: number,
	): Promise<Array<SitecoreCommunityEvent>> {
		const params = [
			`limit=${maxResults}`,
			"offset=0",
			"upcoming=false",
		...
		return fetch(
			...
			return data.result.contents.map(
				(event: SitecoreCommunityEventResponse) => ({
					contentType: "event",
	...
	static async getContent({
		...
		sort,
		forum,
	}: SitecoreCommunityOptions): Promise<Array<SitecoreCommunityContent>> {
		const params = [
			`sort=${sort}`,
			"stFrom=0",
		...
		return fetch(
			...
			.then(async (response) => {
				...
				const data = await response.json();
				return data.result.contents.map(
					(item: SitecoreCommunityContentResponse) => ({
	...
	}: SitecoreCommunityOptions): Promise<
