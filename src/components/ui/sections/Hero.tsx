...
export const Hero = ({ description, title, subTitle, children, productLogo, demoId, portalURL, className }: HeroProps) => {
  ...
  return (
    <VerticalGroup className={cn('max-w-full border-b border-t border-border-color bg-hero-gradient', className)}>
      <CenteredContent className="py-6 px-4 md:py-12 xl:py-16" direction="column">
        ...
        {portalURL && (
          <a href={portalURL} target="_blank" rel="noopener noreferrer" title="Login">
            Login
          </a>
        )}
        {children}
