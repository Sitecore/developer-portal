...
const alertVariants = cva('relative w-full rounded-md px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-4 gap-y-0.5 items-center [&>svg]:size-4 [&>svg]:text-current', {
  variants: {
    variant: {
...
function Alert({ className, variant = 'default', ...props }: React.ComponentProps<'div'> & VariantProps<typeof alertVariants>) {
  return (
    <div data-slot="alert" role="alert" className={cn(alertVariants({ variant }), className)} {...props}>
      {variant && <Icon path={variantIcons[variant]} size={1.2} className="text-current" />}
      {props.children}
    </div>
  );
}
...
function AlertDescription({ className, ...props }: React.ComponentProps<'div'>) {
  return <div data-slot="alert-description" className={cn('text-md col-start-2 grid justify-items-start gap-1 [&_p]:leading-relaxed', className)} {...props} />;
}
