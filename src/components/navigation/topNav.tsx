...
export default function TopNav({ searchEnabled }: TopNavProps) {
  ...
  const isSitecoreLogo: boolean = router.asPath.includes('/roadmap') || router.asPath.includes('/login');
    ...
    <header className="sticky top-0 z-30 w-full px-2 bg-background max-w-none py-0 lg:px-4 shadow-[0_1px_3px_rgba(0,0,0,0.1),0_1px_2px_rgba(0,0,0,0.06)] h-14">
      <div className="flex justify-between items-center mx-auto w-full h-14 lg:gap-x-2">
        ...
        <div className="w-[270px]">
          <span className="text-xl font-bold text-red-500">
            <Link href="/" className="w-auto">
              <Image alt="Logo" className="shrink-0 grow-0 rounded-md object-cover object-left p-1 block" src={getLogoByPath(isSitecoreLogo, theme) || ''} width={isSitecoreLogo ? 130 : 240} height={isSitecoreLogo ? 32 : 40} />
            </Link>
          </span>
                    ...
                    {navItem.type === 'grouped' ? (
                              ...
                              <div key={item.url || item.title}>
                                ...
                                <ul className={`grid gap-2 ${(item.children?.length ?? 0) > 5 ? 'grid-cols-2' : ''}`}>
                                  {item.children?.map((product) => (
                                    <ProductListItem key={product.url || product.title} href={product.url} title={product?.title}>
                      ...
                      <ul className="grid gap-3 p-4 md:w-[400px] lg:w-[500px] lg:grid-cols-[.75fr_1fr] bg-neutral-fg-inverse">
                        ...
                        {navItem?.children?.slice(1).map((child) => (
                          <ListItem key={child.title} href={child.url} title={child.title} />
                        ))}
                      </ul>
                    ) : null}
                  </NavigationMenuContent>
        ...
        <div className="flex items-center gap-x-0.5 md:gap-x-2 xl:flex-1 xl:justify-end xl:gap-x-3">
          ...
          {searchEnabled && <PreviewSearchInput rfkId="rfkid_6" defaultItemsPerPage={6} />}
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            <SheetTrigger asChild>
              <Button variant="ghost" size="sm" aria-label="Toggle Navigation" className="md:hidden h-9 w-9 p-0">
...
function getLogoByPath(isSitecoreLogo: boolean, theme: string | undefined): string | undefined {
  // Default to developer logo
  let logo: Product = isSitecoreLogo ? Product.Sitecore : Product.SitecoreDevelopers;
    ...
    <nav className="max-h-[calc(100dvh-5rem)] overflow-y-auto px-4 py-2">
      <ul className="flex flex-col">
        {mainNavigation.map((navItem) => {
          ...
          if (navItem.url && !navItem.children) {
            return (
              <li key={key} className="border-b last:border-b-0">
                <Link href={navItem.url} onClick={onNavigate} className="flex items-center py-3 text-base font-medium">
                  {navItem.title}
                </Link>
              </li>
            );
          ...
          return (
            <li key={key} className="border-b last:border-b-0">
              <Collapsible open={isExpanded} onOpenChange={(open) => toggleExpanded(key, open)}>
                <CollapsibleTrigger asChild>
                  <button type="button" className="flex w-full items-center justify-between py-3 text-left text-base font-medium" aria-expanded={isExpanded}>
                    <span>{navItem.title}</span>
                    <ChevronDown aria-hidden="true" className={cn('h-3.5 w-3.5 shrink-0 text-foreground transition-transform duration-200', isExpanded && 'rotate-180')} />
                  </button>
                </CollapsibleTrigger>
                <CollapsibleContent className="pb-3">
                  <MobileNavChildren navItem={navItem} onNavigate={onNavigate} />
                </CollapsibleContent>
              </Collapsible>
            </li>
          );
...
function MobileNavChildren({ navItem, onNavigate }: MobileNavChildrenProps) {
      ...
      <div className="space-y-4 pl-1">
        {navItem.children?.map((group) => (
          <div key={group.url || group.title}>
            {group.title.trim().length > 0 && <p className="mb-2 text-xs uppercase tracking-wide text-muted-foreground">{group.title}</p>}
            <ul className="space-y-1">
              {group.children?.map((product) => (
                <li key={product.url || product.title}>
                  ...
                  >
                    <ProductIcon product={product.logo || Product.Sitecore} height="18px" width="30px" />
      ...
      <ul className="space-y-1 pl-1">
        {navItem.children?.map((child) => (
          <li key={child.url || child.title}>
            <Link href={child.url || '#'} onClick={onNavigate} target={child.external ? '_blank' : undefined} rel={child.external ? 'noopener noreferrer' : undefined} className="block rounded-md px-2 py-2 hover:bg-accent">
              <span className="text-sm font-medium">{child.title}</span>
              {child.subTitle && <p className="mt-1 text-xs text-muted-foreground line-clamp-2">{child.subTitle}</p>}
            </Link>
          </li>
        ))}
      </ul>
  ...
  return (
    <ul className="space-y-1 pl-1">
      {navItem.children?.map((child) => (
        <li key={child.url || child.title}>
          <Link href={child.url || '#'} onClick={onNavigate} target={child.external ? '_blank' : undefined} rel={child.external ? 'noopener noreferrer' : undefined} className="block rounded-md px-2 py-2 text-sm hover:bg-accent">
            {child.title}
          </Link>
        </li>
      ))}
