    ...
    <Card style="flat" elevation="none" padding="sm">
      <CardHeader className="space-y-3">
        ...
        {hasImage ? (
          <Link
            href={post.href}
            className="relative block aspect-[2/1] w-full max-w-2xl overflow-hidden rounded-lg border border-border-color bg-muted"
...
export function BlogOverviewClient({
  ...
  const authorSlug =
    router.isReady && typeof router.query.author === "string"
      ...
      : "";
  const pageRaw =
    router.isReady && typeof router.query.page === "string"
  ...
  const { pageItems, totalPages, totalItems } = useMemo(
    () => paginate(filtered, page, BLOG_PAGE_SIZE),
    [filtered, page],
  );
